 % main_simulate_echoes_with_array.m
% 雷达目标信号回波模拟仿真程序
...
% --- 1.2 仿真控制 ---
frame_index_to_save = 0; % 定义要保存的仿真数据帧的编号
...
% =========================================================================
% 描述: 此部分根据雷达参数，生成一个完整的、由三段不同类型脉冲拼接而成的发射波形。
%       这个波形是后续生成目标回波的基础。

%% 3. 生成发射参考波形
% =========================================================================
% --- 3.1 【修正】根据实际脉冲宽度和采样率计算信号点数 ---
...
pulse1_prt_s = [pulse1, zeros(1,285-4)]; 
pulse2_prt_m = [pulse2, zeros(1, 795-200)];
pulse3_prt_l = [pulse3, zeros(1, 3825-700)];

% --- 3.4 将短脉冲放置在长的采样波门内，并拼接成一个完整的发射脉冲 ---
tx_pulse = zeros(1, config.Sig_Config.point_PRT);  % 初始化总发射脉冲向量 (长度3404)

% 1. 填充窄脉冲段
% 将长度为 4 的 pulse1 放置在第一个采样波门 (长度228) 的开头
tx_pulse(1:num_samples_1) = pulse1;

% 2. 填充中脉冲段
% 计算第二个采样波门的起始点
offset1 = config.Sig_Config.point_prt_segments(1); % 228
% 将长度为 200 的 pulse2 放置在第二个采样波门的开头
tx_pulse(offset1+1 : offset1+num_samples_2) = pulse2;

% 3. 填充长脉冲段
% 计算第三个采样波门的起始点
offset2 = offset1 + config.Sig_Config.point_prt_segments(2); % 228 + 723
% 将长度为 700 的 pulse3 放置在第三个采样波门的开头
...
% 
% subplot(2,1,2);
% plot(t, unwrap(angle(z))); % 绘制相位，使用unwrap避免相位跳变
% xlabel('时间');
% ylabel('弧度');
% title('复数信号相位时域图');
%% 4. 模拟多通道回波信号
...
save(output_filename, 'raw_iq_data', 'servo_angle');
...
echo_for_pc = squeeze(raw_iq_data(:, :, 6)); 
...
% 4. 使用FFT进行快速卷积
pc_result = ifft(fft(echo_for_pc, [], 2) .* repmat(fft(matched_filter), 332, 1), [], 2);                     % 不加窗脉压结果
pc_result_windowed = ifft( fft(echo_for_pc, [], 2) .* repmat(fft(matched_filter_windowed), 332, 1), [], 2 ); % 加窗脉压结果
...
figure;
subplot(3,1,1);
plot(1:285, pulse1_prt_s);
subtitle('短脉冲波形');
xlabel('点数');
ylabel('幅度');

subplot(3,1,2);
plot(1:795, real(pulse2_prt_m));
subtitle('中脉冲波形');
xlabel('点数');
ylabel('幅度');

subplot(3,1,3);
plot(1:3825, real(pulse3_prt_l));
subtitle('长脉冲波形');
xlabel('点数');
ylabel('幅度');

figure;
pulse_all = [pulse1_prt_s, pulse2_prt_m, pulse3_prt_l];
noise_power1 = 1e-3; % 可调噪声功率
noise1 = sqrt(noise_power1/2) * (randn(size(pulse_all)) + 1j * randn(size(pulse_all)));
%raw_iq_data = raw_iq_data*10;
pulse_all = pulse_all + noise1;
plot(1:length(pulse_all), real(pulse_all));
subtitle('长脉冲波形');
xlabel('点数');
ylabel('幅度');
...
plot(range_axis_pc, 20*log10(abs(pc_result)));
grid on; 
hold on;
...
xlabel('距离 (m)');
ylabel('幅度');
xlim([0, max([targets.Range])*1.2]);
hold off;

subplot(2,1,2);
range_axis_pc_windowed = (0:length(pc_result_windowed)-1) * deltaR;
plot(range_axis_pc_windowed, 20*log10(abs(pc_result_windowed)));
grid on; 
hold on;
for i = 1:length(targets)
    xline(targets(i).Range, '--r', sprintf('目标 %d', i));
...
% 速度剖面图
% 1. 创建一个新的图形窗口
figure('Name', 'MTD 速度维剖面');
% 2. 找到RDM中能量最强的点，以该点所在的距离单元进行切片
[~, max_idx] = max(abs(rdm_windowed(:))); % 找到能量最大点的线性索引
[max_v_idx, max_r_idx] = ind2sub(size(rdm_windowed), max_idx); % 转换为二维(行,列)索引

% 3. 提取该距离单元（即RDM矩阵的一整列）的速度维数据
velocity_profile = abs(rdm_windowed(:, max_r_idx));
% 4. 绘制速度剖面图
plot(velocity_axis, 20*log10(velocity_profile)); % 将幅度转换为dB单位显示
% 5. 美化图形
grid on;
% title(sprintf('速度维剖面图 @ 距离 = %.2f m', range_axis(max_r_idx)));
title(sprintf('速度维剖面图'));
xlabel('速度 (m/s)');
ylabel('幅度 (dB)');
hold on;
peak_velocity = velocity_axis(max_v_idx);
% xline(peak_velocity, '--r', sprintf('峰值速度 @ %.2f m/s', peak_velocity)); % 在峰值速度处画一条垂直虚线
xline(peak_velocity, '--r', 'Label', sprintf('峰值速度 @ %.2f m/s', peak_velocity), 'LabelVerticalAlignment', 'bottom', 'LabelOrientation', 'horizontal');
hold off;

% 子图6：三维RD图
figure;
[RangeGrid, DopplerGrid] = meshgrid(range_axis_pc_windowed, velocity_axis);
surf(RangeGrid, DopplerGrid, 20*log10(abs(rdm_windowed)));
title('三维 RD 图');
xlabel('多普勒 (Hz)');
ylabel('距离 (m)');
zlabel('幅度/能量');
view(45, 30); % view(方位角, 仰角)
% 启用色彩映射
colormap('jet'); % 使用 'jet' 色彩映射，您也可以尝试其他如 'hot', 'parula'
colorbar; % 显示颜色条
