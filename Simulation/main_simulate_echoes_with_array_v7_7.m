% main_simulate_echoes_with_array_v7_7.m
% 版本: v7.7
...
%  25/10/21   XZR      V7.5     固定噪声功率作为基底，增添一个检测目标，实现对两个目标的检测，以验证其分辨率
%  25/10/29   XZR      V7.6     修改优化了了画图程序，现在各个脉冲全程处理波峰能够在距离上对齐了
%  25/11/04   XZR      V7.7     重写了聚类归并算法，现在是二级聚类归并算法，第一级加权平均波束内归并，第二级赢家通吃（选大）波束间归并
...
% --- 1.1 定义仿真的目标 ---
% 首先清除可能存在的旧 'targets' 变量
clear targets;

% --- 定义目标 1 ---
targets(1).Range = 10000;         % 目标1 距离 (m)
targets(1).Velocity = 5;         % 目标1 速度 (m/s)
targets(1).RCS = 1;               % 目标1 雷达散射截面 (m^2)
targets(1).ElevationAngle = 10;   % 目标1 俯仰角 (度)
targets(1).SNR_dB = -10;           % 目标1 信噪比 (dB)

% --- 定义目标 2 ---
targets(2).Range = 5000;         % 目标2 距离 (m)
targets(2).Velocity = 8;       % 目标2 速度 (m/s)
targets(2).RCS = 1;               % 目标2 雷达散射截面 (m^2)
targets(2).ElevationAngle = 15;   % 目标2 俯仰角 (度)
targets(2).SNR_dB = -10;           % 目标2 信噪比 (dB)
% 
% --- 定义目标 3 ---
targets(3).Range = 8000;         % 目标2 距离 (m)
targets(3).Velocity = 10;       % 目标2 速度 (m/s)
targets(3).RCS = 1;               % 目标2 雷达散射截面 (m^2)
targets(3).ElevationAngle = 4;   % 目标2 俯仰角 (度)
targets(3).SNR_dB = -10;           % 目标2 信噪比 (dB)


P_noise_floor = 1; % 噪声基底功率
% SNR_dB = -10; % 这是全局SNR，信噪比 功率（20dB-100倍，10dB-10倍，3dB-1倍）

prt_selected = 5;   % 选择后续用于画图分析的prt   
beam_selected = 5; % 选择后续用于画图分析的波束
...
% --- 1.4：聚类参数配置 ---
cluster_params.max_range_sep = 30;  % 距离维上，点被视为同一簇的最大间隔 (米)
cluster_params.max_vel_sep = 2;   % 速度维上，点被视为同一簇的最大间隔 (米/秒)
...
deltaR = config.Sig_Config.c*ts/2; % 采样距离长度
deltaV = lamda*deltaDoppler/2;     % 速度分辨率

R1 = config.Sig_Config.c/config.Sig_Config.B*2;   % 距离分辨率
FFTnum_MTD = 512; % MTD处理中FFT点数（补零后）
...
% 计算画图用距离轴和速度轴
% R_rx = linspace(30, 30 + 3404*6, 3404);
v_max = config.Sig_Config.wavelength / (2 * config.Sig_Config.prt);  % 计算雷达最大不模糊速度
velocity_axis = linspace(-v_max/2, v_max/2, FFTnum_MTD);             % 计算雷达速度轴，轴有正负半轴时，vmax要除以2
range_axis = (1:config.Sig_Config.point_PRT) * (config.Sig_Config.c / (2 * config.Sig_Config.fs)); % 计算雷达距离轴
% range_axis = R_rx-24;
...
for m = 1:config.Sig_Config.prtNum % 循环生成每一个prt的信号  
    ...
    for k = 1:length(targets)
        ...
        % --- 根据每个目标的信噪比(SNR)计算其所需幅度 ---
        % 定义 "噪声基底" 功率：设定基底功率为 1，所有信号的SNR都将参考这个值。
        % P_noise_floor = 1;
        % 从 targets 结构体中获取当前目标 k 的信噪比 (线性值)
        SNR_k_lin = 10^(targets(k).SNR_dB / 10);

        % 计算这个目标 k 所需的信号功率 (P_sig = SNR * P_noise)
        P_signal_k = SNR_k_lin * P_noise_floor;

        % 计算所需的幅度缩放因子 A
        %    P_signal_k = A^2 * P_signal_unscaled
...
% noise_power = P_noise_floor;                              % 高斯白噪声功率
...
% DBF前对16通道信号数据加入高斯白噪声，利用for循环对每个通道添加不相关的高斯白噪声
% 噪声由伪随机数种子生成，一次性添加噪声三维矩阵似乎会在不同通道的信号中产生相关性
...
for c = 1:16
    ...
    noise = noise .* sqrt(P_noise_floor/2);                   % 双路IQ信号功率各占总功率一半，平方根是把功率值转换为幅度值
...
DBF_coeffs_data_C_fliplr = fliplr(DBF_coeffs_data_C);
% --- 逐脉冲DBF ---
...
for i = 1:config.Sig_Config.prtNum
    ...
    single_pulse_13beam = single_pulse_16ch * DBF_coeffs_data_C_fliplr.';
    iq_data_13beam(i, :, :) = single_pulse_13beam;
end

% --- DBF结果可视化 ---
figure('Name', '阶段1: DBF后波束功率');
beam_to_check = 5; % 选择一个靠近目标的波束来显示
...
tx_start_narrow = 1;                                      % 窄脉冲发射沿起始处 第1点
tx_start_medium = num_samples_1 + gap_duration1_num + 1;  % 中脉冲发射沿起始处 第290点
tx_start_long = num_samples_1 + gap_duration1_num + num_samples_2 + gap_duration2_num + 1;  % 长脉冲采样波门起始处 第1285点

seg_start_narrow = tx_start_narrow + num_samples_1; % 窄脉冲采样波门起始处 第5点
seg_start_medium = tx_start_medium + num_samples_2; % 中脉冲采样波门起始处 第490点
seg_start_long =  tx_start_long + num_samples_3;    % 长脉冲采样波门起始处 第1985点
...
for b = 1:config.Sig_Config.beam_num
    ...
    % 画窄脉冲滤波前后及滤波延迟校正后的信号波形图（可注释）
    % figure;
    % plot(real(segment_narrow(1,:)),'b');
    % hold on;
    % plot(real(pc_out_narrow_full(1,:)),'r');
    % plot(real(pc_out_narrow_full_shift(1,:)),'y');
    % hold off;
    % legend('原始信号','FIR滤波后信号','FIR滤波并校正延迟后信号')
    ...
    piece1 = pc_out_narrow_full_shift(:, 1: N_gate_narrow);   % 取窄脉冲处理后信号的5-232点（共计228点）
    % 中脉冲结果段
    % 注意：这里我们截取的是中脉冲处理结果中，紧接着窄脉冲之后的部分
    idx2_start = N_gate_narrow + 1;
    idx2_end   = N_gate_narrow + N_gate_medium;
    piece2 = pc_out_medium_full(:, idx2_start:idx2_end);      % 取中脉冲处理后信号的229-951点（共计723点）

    % 长脉冲结果段
    idx3_start = N_gate_narrow + N_gate_medium + 1;
    idx3_end   = N_gate_narrow + N_gate_medium + N_gate_long;
    piece3 = pc_out_long_full(:, idx3_start:idx3_end);        % 取长脉冲处理后信号的952-3404点（共计2453点）
...
% 对13个波束的脉压结果，逐一进行MTD
rdm_13beam = complex(zeros(FFTnum_MTD, 3404, 13));
...
for b = 1:config.Sig_Config.beam_num
    ...
    rdm_13beam(:,:,b) = fftshift(fft(pc_data_single_beam_win, FFTnum_MTD, 1), 1);
...
if num_total_raw_detections > 0
    ...
    % range_axis = (1:N_total_gate) * (config.Sig_Config.c / (2 * config.Sig_Config.fs));
    ...
    for i = 1:num_total_raw_detections
        ...
        if length(vCellsFix) < 3
            ...
            vCellsFixQ = vCellsFix(1): 1/vInterpTimes :vCellsFix(end);   % 速度维插值变量
        ...
        % --- 角度估计 (单脉冲测角) ---
        % (测角部分不变，仍然使用原始整数索引 v_idx, r_idx 查找复数值)，角度插值需要使用二维interp2插值处理

        beam_A_idx = pair_idx;
        beam_B_idx = pair_idx + 1;
        S_A = abs(rdm_13beam(v_idx, r_idx, beam_A_idx));
        S_B = abs(rdm_13beam(v_idx, r_idx, beam_B_idx));
        % S_A = rdm_13beam(vCellMax, rCellMax, beam_A_idx);
        % S_B = rdm_13beam(vCellMax, rCellMax, beam_B_idx);
        ...
        angle_offset = k_slope * monopulse_ratio;
...
% 10.1 第一级: 波束【内】聚类 (R/V/A 分组, 功率加权平均)
% =========================================================================
% 职责: 将同一个波束对检测到的、R/V/A均相近的"检测点点云"合并为一个更精确的点。
fprintf('--- 10.1 执行波束【内】聚类 (R/V/A分组, 加权平均) ---\n');
...
if num_total_raw_detections > 0
    ...
    fprintf('  > 第1级聚类完成，将 %d 个精测点合并为 %d 个波束内目标。\n', num_total_raw_detections, num_clusters_stage1);
    ...
    for i = 1:num_clusters_stage1
        ...
        final_PairIndex = mode([detections_in_cluster.PairIndex]);

        intra_beam_targets(i).Range = final_range;
        intra_beam_targets(i).Velocity = final_velocity;
        intra_beam_targets(i).Angle = final_angle;
        intra_beam_targets(i).Power = total_power; % 存储总功率
        intra_beam_targets(i).PairIndex = final_PairIndex;
    ...
    fprintf('\n--- 波束内聚类目标列表 (一级聚类后) ---\n');
    for i = 1:num_clusters_stage1
        fprintf('目标 %d: 距离 ~%.2f m, 速度 ~%.2f m/s, 估算角度 ~%.2f 度 (功率 %.2f)\n', ...
            i, intra_beam_targets(i).Range, intra_beam_targets(i).Velocity, intra_beam_targets(i).Angle, intra_beam_targets(i).Power);
    end

else
    fprintf('  > 无原始点迹，跳过波束内聚类。\n');
    intra_beam_targets = [];
end

% 10.2 第二级: 波束【间】聚类 (R/V 分组, 赢家通吃)
% =========================================================================
% 职责: 解决"鬼影"问题。将 R/V 相近但 A 不同的点
%       (来自不同波束对) 合并，并只保留功率最高的那个"正确点"。
fprintf('--- 10.2 执行波束【间】聚类 (R/V分组，保留信号功率最高点) ---\n');
...
if num_intra_beam_targets > 0
    ...
    fprintf('  > 第2级聚类完成，将 %d 个波束内目标合并为 %d 个唯一目标。\n', num_intra_beam_targets, num_clusters_stage2);
    ...
    for i = 1:num_clusters_stage2
        ...
        final_targets(i).Height = final_targets(i).Range * sind(final_targets(i).Angle);
        final_targets(i).Power = winner_detection.Power; % 存储最强点的功率
        final_targets(i).PairIndex = winner_detection.PairIndex;
    end
    
    % --- 打印最终目标列表 ---
    fprintf('\n--- 最终目标列表 (两级聚类后) ---\n');
    for i = 1:num_clusters_stage2
        fprintf('目标 %d: 距离 ~%.2f m，速度 ~%.2f m/s，高度 ~%.2f m，估算角度 ~%.2f 度 (功率 %.2f)\n', ...
            i, final_targets(i).Range, final_targets(i).Velocity, final_targets(i).Height, final_targets(i).Angle, final_targets(i).Power);
    end
    
else
    fprintf('  > 无波束内目标，跳过波束间聚类。\n');
    final_targets = [];
end
...
figure;
subplot(4,1,1);
plot(range_axis, real(pc_results_13beam(prt_selected,:,beam_selected))); 
title('脉压后拼接结果图');
subtitle('实部');
xlabel('距离（m）');
ylabel('信号幅度');
...
subplot(4,1,2);
plot(range_axis, imag(pc_results_13beam(prt_selected,:,beam_selected))); 
title('虚部');
xlabel('距离（m）');
ylabel('信号幅度');
for i=1:length(targets)
...
subplot(4,1,3);
plot(range_axis, abs(pc_results_13beam(prt_selected,:,beam_selected))); 
title('幅值');
xlabel('距离（m）');
ylabel('信号幅度');
for i=1:length(targets)
...
subplot(4,1,4);
plot(range_axis, 20*log10(abs(pc_results_13beam(prt_selected,:,beam_selected)))); 
title('幅值（dB）');
xlabel('距离（m）');
ylabel('信号幅度（dB）');
for i=1:length(targets)
...
% --- 定义用于绘图的坐标轴 ---
% v_max = config.Sig_Config.wavelength / (2 * config.Sig_Config.prt); % 计算雷达最大不模糊速度
% velocity_axis = linspace(-v_max/2, v_max/2, FFTnum_MTD);
...
[~, velocity_unit] = min(abs(velocity_axis-targets(1).Velocity));     % 找到符合设定的目标速度的速度单元
...
% 图 7.3：绘制距离单元剖面图
% 选择一个特定的距离单元来绘制剖面图
distance_unit = round(targets(1).Range/6);  % 选择距离单元，假设为第100个距离点
...
%% 11. 最终结果可视化
% =========================================================================
fprintf('--- 11. 生成最终可视化图表 ---\n');
% --- 11.1 和波束的幅值与CFAR动态检测门限图 ---
...
subplot(2,1,1);
v_max = config.Sig_Config.wavelength / (2 * config.Sig_Config.prt);
imagesc(range_axis, velocity_axis, rdm_for_cfar_all(:,:,beam_selected));
...
% 在图上标记检测到的目标
if num_clusters_stage2 > 0
    detected_ranges = range_axis(detected_r_idx); % 这里点的数据应该是聚类后的
...
%% 绘制所有一次聚类后检测点的距离俯仰角度波束图
% =========================================================================
figure('Name', '所有一次聚类后检测点 (距离-角度)');
...
if isempty(parameterized_detections)
    ...
    % --- 1. 正确地从结构体数组中提取数据 ---
    % 使用 [struct_array.FieldName] 语法可以一次性提取所有同名字段的数据
    ranges = [intra_beam_targets.Range];
    Angles = [intra_beam_targets.Angle];
    pair_indices = [intra_beam_targets.PairIndex];

    % --- 2. 使用scatter函数绘制点迹图 ---
    % X轴: 距离
    % Y轴: 俯仰角度
    % 颜色: 由波束对索引决定
    scatter(ranges, Angles, 50, pair_indices, 'filled');

    % --- 3. 添加标注和颜色条 ---
    grid on;
    xlabel('距离 (m)');
    ylabel('俯仰角度 (度)');
    title(sprintf('所有脉冲内聚类归并后检测点迹 (%d 个点)', length(ranges)));
...
%% 绘制所有二次聚类后检测点的距离俯仰角度波束图
% =========================================================================
figure('Name', '所有二次聚类后检测点 (距离-角度)');
...
if isempty(parameterized_detections)
    ...
    % --- 1. 正确地从结构体数组中提取数据 ---
    % 使用 [struct_array.FieldName] 语法可以一次性提取所有同名字段的数据
    ranges = [final_targets.Range];
    Angles = [final_targets.Angle];
    pair_indices = [final_targets.PairIndex];
    ...
    % 颜色: 由波束对索引决定
    % scatter(ranges, Angles, 50, pair_indices, 'filled');
    % colormap('parula'); % <-- 强制使用一个标准 Colormap
    scatter(ranges, Angles, 50, 'filled');
    
    % --- 3. 添加标注和颜色条 ---
    grid on;
    xlabel('距离 (m)');
    ylabel('俯仰角度 (度)');
    title(sprintf('所有脉冲间聚类归并后检测点迹 (%d 个点)', length(ranges)));
...
beam_data_s_temp = beam_data_temp(:, 1:end);
% 窄脉冲 (时域FIR滤波)
% 非加窗
fir_coeffs = MF_narrow;
% filter函数会逐行处理矩阵
pc_out_narrow_full_temp = filter(fir_coeffs, 1, beam_data_s_temp, [], 2);
% 校正FIR引入的延迟
...
% filter函数会逐行处理矩阵
pc_out_narrow_full_temp_win = filter(fir_coeffs_windowed, 1, beam_data_s_temp, [], 2);
% 校正FIR引入的延迟
pc_out_narrow_full_temp_win = circshift(pc_out_narrow_full_temp_win, -fir_delay, 2);


beam_data_m_temp = beam_data_temp(:, 490:end);
% 中脉冲 (非加窗时域卷积)
pc_out_medium_full_temp = complex(zeros(size(beam_data_m_temp,1), size(beam_data_m_temp,2) + num_samples_2 - 1));
for i = 1:config.Sig_Config.prtNum
    pc_out_medium_full_temp(i,:) = conv(beam_data_m_temp(i, :), MF_medium);
end
% 中脉冲 (加窗时域卷积)
pc_out_medium_full_temp_win = complex(zeros(size(beam_data_m_temp,1), size(beam_data_m_temp,2) + num_samples_2 - 1));
for i = 1:config.Sig_Config.prtNum
    pc_out_medium_full_temp_win(i,:) = conv(beam_data_m_temp(i,:), MF_medium_win);
end

beam_data_l_temp = beam_data_temp(:, 1985:end);
% 长脉冲 (非加窗时域卷积)
pc_out_long_full_temp = complex(zeros(size(beam_data_l_temp,1), size(beam_data_l_temp,2) + num_samples_3 - 1));
for i = 1:config.Sig_Config.prtNum
    pc_out_long_full_temp(i,:) = conv(beam_data_l_temp(i, :), MF_long);
end
% 长脉冲 (加窗时域卷积)
pc_out_long_full_temp_win = complex(zeros(size(beam_data_l_temp,1), size(beam_data_l_temp,2) + num_samples_3 - 1));
for i = 1:config.Sig_Config.prtNum
    pc_out_long_full_temp_win(i,:) = conv(beam_data_l_temp(i, :), MF_long_win);
end
