% main_simulate_echoes_with_array_v7_1.m
% 版本: v7.1
...
% --- 1.4：聚类参数配置 ---
cluster_params.max_range_sep = 2;  % 距离维上，点被视为同一簇的最大间隔（单元数）
cluster_params.max_vel_sep = 2;    % 速度维上，点被视为同一簇的最大间隔（单元数）
cluster_params.max_beam_sep = 2;   % 波束维上，点被视为同一簇的最大间隔
...
%% 9. 目标聚类 (含波束归并)
% =========================================================================
fprintf('--- 9. 执行目标聚类/波束归并 ---\n');
num_raw_detections = size(all_raw_detections, 1);  % 获取多波束CFAR输出的原始点迹总数
clusters = {};                                     % 初始化最终的簇列表，用来存储最终的聚类结果

if num_raw_detections > 0
    % 聚类参数
    max_range_sep = cluster_params.max_range_sep;    % 距离维上，点被视为同一簇的最大间隔（单元数）
    max_vel_sep = cluster_params.max_vel_sep;        % 速度维上，点被视为同一簇的最大间隔（单元数）
    max_beam_pair_sep = cluster_params.max_beam_sep; % 波束对索引的最大间隔

    % --- 聚类算法核心 ---
    cluster_IDs = zeros(num_raw_detections, 1);      % 创建一个数组，用于存储每个点迹所属的簇ID，0表示未分配
    current_cluster_ID = 0;                          % 初始化簇的编号
    
    % 遍历所有点迹，执行三维聚类 (速度, 距离, 波束对)
    for i = 1:num_raw_detections
        if cluster_IDs(i) == 0   % 如果当前点尚未被分配给任何簇
            current_cluster_ID = current_cluster_ID + 1;  % 创建一个新簇，编号加1
            points_to_visit = i; % 将当前点作为"种子"放入待访问列表（可以理解为一个栈）
            
            % 开始邻域搜索，只要待访问列表不为空就一直循环
            while ~isempty(points_to_visit)
                current_idx = points_to_visit(1); % 取出列表中的第一个点
                points_to_visit(1) = [];          % 从列表中移除（出栈）
                if cluster_IDs(current_idx) == 0  % 检查，防止因入栈时未分配ID而导致的重复处理
                    cluster_IDs(current_idx) = current_cluster_ID; % 将当前点正式分配给当前的簇
                    
                    % 遍历所有其他点，寻找当前点的"邻居"
                    for j = 1:num_raw_detections  
                        if cluster_IDs(j) == 0    % 只在未分配的点中寻找
                            % 计算两个点在三个维度上的距离（单位：单元数）
                            dist_v = abs(all_raw_detections(current_idx, 1) - all_raw_detections(j, 1));
                            dist_r = abs(all_raw_detections(current_idx, 2) - all_raw_detections(j, 2));
                            dist_b = abs(all_raw_detections(current_idx, 3) - all_raw_detections(j, 3));
                            
                            % 如果一个点在速度、距离和波束维度上都足够近，则认为它们属于同一个目标，应归为一类
                            if dist_v <= max_vel_sep && dist_r <= max_range_sep && dist_b <= max_beam_pair_sep
                                points_to_visit(end+1) = j; % 将这个新邻居加入待访问列表（入栈），以便后续以它为中心继续搜索
                            end
                        end
                    end
                end
    ...
    num_clusters = current_cluster_ID; % 最终识别出的簇（唯一目标）的数量
    fprintf('聚类完成，将 %d 个原始点迹合并为 %d 个唯一目标。\n', num_raw_detections, num_clusters);
    
    % 将聚类结果打包成 cell 数组，以供下一阶段使用
    clusters = cell(num_clusters, 1);
    for i = 1:num_clusters
        cluster_mask = (cluster_IDs == i);                 % 找到所有ID为i的点
        clusters{i} = all_raw_detections(cluster_mask, :); % 将这些点的信息存入对应的cell中
    end
else
    fprintf('未检测到原始点迹，无需聚类。\n');
end

    
%% 10. 对每个聚类簇进行参数精估计 (含单脉冲测角)
% =========================================================================
% 该部分先利用功率加权平均（质心法）和插值求解更精确的物理量，再用和差比幅单脉冲测角法解算俯仰角
if num_clusters > 0
    final_targets = struct('Range', cell(1,num_clusters), 'Velocity', cell(1,num_clusters), 'Angle', cell(1,num_clusters));  % 初始化最终目标物理信息存储结构体
    fprintf('\n--- 最终目标列表 (精估计后) ---\n');

    % 遍历上一步聚类得到的每一个目标簇
    for i = 1:num_clusters
        % 从聚类结果中，提取出属于当前簇(ID=i)的所有原始点迹
        points_in_cluster_mask = (cluster_IDs == i);
        points_in_cluster_indices = all_raw_detections(points_in_cluster_mask, :);
        % 提取簇内所有点的线性功率值 (注意: 不是dB值)
        linear_powers = zeros(size(points_in_cluster_indices, 1), 1);
        for p_idx = 1:size(points_in_cluster_indices, 1)
            v_idx = points_in_cluster_indices(p_idx, 1);       % 速度索引
            r_idx = points_in_cluster_indices(p_idx, 2);       % 距离索引
            linear_powers(p_idx) = rdm_for_cfar(v_idx, r_idx); % 从和波束RDM中提取功率（这里用信号幅值代替）
        end
        total_power = sum(linear_powers);
        
        % --- 10.1 距离和速度精估计：计算功率加权的质心 ---
        % 公式: 质心索引 = Σ(每个点的索引 * 每个点的功率) / Σ(总功率)
        ...
        % 使用interp1进行线性插值，将浮点数索引转换为精确的物理量（米 和 米/秒）
        final_range = interp1(1:num_R, range_axis, centroid_r_idx);
        final_velocity = interp1(1:num_V, velocity_axis, centroid_v_idx);

        % --- 10.2 单脉冲测角 ---
        % 使用簇的质心索引(四舍五入到最近的整数)进行后续的测角计算
        v_idx = round(centroid_v_idx); % 使用簇中心的索引
        r_idx = round(centroid_r_idx);
       
        % 遍历所有13个原始波束，找到在簇中心位置信号幅度最强的那个"主波束"
        all_beams_power_at_target = zeros(1, config.Sig_Config.beam_num);
        for b = 1:config.Sig_Config.beam_num
            rdm = squeeze(rdm_13beam(:,:,b));                      % 提取单个波束的RDM
            all_beams_power_at_target(b) = abs(rdm(v_idx, r_idx)); % 记录该波束下信号幅度
        end
        [~, main_beam_idx] = max(all_beams_power_at_target);       % 找到幅度最大值对应的波束索引
       
        % 根据主波束位置，选择它和它信号更强的那个邻居组成测角波束对 (A, B)
        if main_beam_idx == 1, beam_A_idx = 1; beam_B_idx = 2; % 边界处理：如果主波束在最左边
        elseif main_beam_idx == config.Sig_Config.beam_num, beam_A_idx = config.Sig_Config.beam_num - 1; beam_B_idx = config.Sig_Config.beam_num; % 边界处理：主波束在最右边
        else % 中间情况：比较左右相邻波束的功率（这里用信号幅值代替）
            power_left = abs(rdm_13beam(v_idx, r_idx, main_beam_idx - 1));
            power_right = abs(rdm_13beam(v_idx, r_idx, main_beam_idx + 1));
            if power_left > power_right     % 左邻波束幅度更强
        ...
        % 提取波束A和B在该点的幅值，计算和波束和差波束数值，再计算和差比
        S_A = abs(rdm_13beam(v_idx, r_idx, beam_A_idx));
        S_B = abs(rdm_13beam(v_idx, r_idx, beam_B_idx));
        % 核心公式: 计算和差比(单脉冲比)
        monopulse_ratio = (S_A - S_B) / (S_A + S_B);
        
        % 角度解算
        % 加载预先知道的波束指向(来自方向图)和标定好的K值查找表
        beam_angles_deg = [-16, -9.6, -3.2, 3.2, 9.6, 16, 22.6, 29.2, 36.1, 43.3, 51, 59.6, 70.3];
        k_slopes_LUT = [-4.6391,-4.6888,-4.7578,-4.7891,-4.7214,-4.7513,-5.2343,-5.4529,-5.7323,-6.1685,-7.0256,-8.7612]; % 利用先前标定好的鉴角曲线（k值）
        % 从查找表中取出当前波束对(A,B)对应的精确K值
        k_slope = k_slopes_LUT(beam_A_idx);    % (注意：LUT的索引对应的是波束对的第一个波束的索引, 即beam_A_idx)
        angle_A = beam_angles_deg(beam_A_idx);
        angle_B = beam_angles_deg(beam_B_idx);
        % 最终角度 = 波束对中心角度 + 角度偏移量(K * 单脉冲比实部)
        angle_offset = k_slope * real(monopulse_ratio);
        final_angle = (angle_A + angle_B)/2 + angle_offset;
...
fprintf('聚类后结果：目标 %d: 距离 ~%.2f m, 速度 ~%.2f m/s\n', i, final_range, final_velocity);
