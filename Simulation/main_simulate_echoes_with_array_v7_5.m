% main_simulate_echoes_with_array_v7_5.m
% 版本: v7.5
...
targets(1).ElevationAngle = 5;   % 目标1 俯仰角 (度)
targets(1).SNR_dB = 30;           % 目标1 信噪比 (dB)

% --- 定义目标 2 ---
targets(2).Range = 8100;         % 目标2 距离 (m)
targets(2).Velocity = 15;         % 目标2 速度 (m/s)
targets(2).RCS = 1;               % 目标2 雷达散射截面 (m^2)
targets(2).ElevationAngle = 10;   % 目标2 俯仰角 (度)
targets(2).SNR_dB = 30;           % 目标2 信噪比 (dB)
...
cluster_params.max_vel_sep = 0.4;   % 速度维上，点被视为同一簇的最大间隔 (米/秒)
...
for b = 1:config.Sig_Config.beam_num
    ...
    piece1 = pc_out_narrow_full_shift(:, 1+cutFir_start : N_gate_narrow+cutFir_start);   % 取窄脉冲处理后信号的5-232点（共计228点）
    % 中脉冲结果段
