% main_multiframe_simulation_v9_Scan.m (v9.1)
%
% 描述:
%   这是一个多帧雷达仿真主脚本。
%   (v9.1 升级): 增加了伺服机构的模拟。
%
% 架构:
%   %% 0. 多帧仿真配置 (新增 config.scan)
%   %% 1-3. 一次性设置 (计算 deg_per_frame)
%   %% 4.   主循环
%           4.1. 状态演进 (目标移动 + *天线旋转*)
%           4.2. 调用处理核
...
%% 0. 多帧仿真配置
% =========================================================================
total_frames_to_run = 5;  % <--- 在此设置要模拟的总帧数
...
deg_per_sec = config.scan.rpm * (360 / 60); % e.g., 6 * 6 = 36 deg/sec
config.scan.deg_per_frame = deg_per_sec * T_frame; % e.g., 36 * 0.077 = 2.77 deg/frame
...
% (复制 %% 3.1 到 %% 3.7 的所有 v9 代码, 此处省略以保持简洁)
% ... (v9 %% 3.1)
...
for frame_idx = 1 : total_frames_to_run
    ...
    % --- 4.2. 运行 v8 的处理核 ---
    % (fun_process_single_frame 保持不变, 它不需要知道方位角)
    ...
    if ~isempty(final_targets)
        ...
        [final_targets.iAntAngle] = deal(ant_angle_cell{:}); % <--- (新) 注入方位角
...
if ~isempty(cumulative_final_log)
    ...
    % 提取所有目标的 R, V, El, Az, Frame
    all_ranges = [cumulative_final_log.Range];
    all_velocities = [cumulative_final_log.Velocity];
    all_angles_el = [cumulative_final_log.Angle];
    all_angles_az = [cumulative_final_log.iAntAngle];
    ...
    % --- (新) 绘制 方位-帧号 (时间) 航迹图 ---
    subplot(2,1,2);
    scatter(all_frames, all_angles_az, 30, all_velocities, 'filled');
    xlabel('帧号 (iFrame)');
    ylabel('方位角 (度)');
    title('天线扫描 (方位角 vs. 时间)');
    grid on;
    set(gca, 'XLim', [0, total_frames_to_run+1]);
    set(gca, 'YLim', [0, 360]);
    h_cb = colorbar;
    ...
    % --- (新) 绘制 距离-方位角 (PPI) 航迹图 ---
    figure('Name', '多帧仿真航迹 (PPI 视图)');
    polarscatter(deg2rad(all_angles_az), all_ranges, 30, all_frames, 'filled');
    % (可选) 设置 r 轴 0 在底部
    % set(gca, 'ThetaZeroLocation', 'bottom'); 
    title('目标航迹 (距离 vs. 方位角)');
