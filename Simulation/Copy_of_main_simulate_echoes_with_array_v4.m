...
noise_power = mean(abs(tx_pulse(tx_pulse ~= 0)).^2) / (10^(SNR_dB / 10));                 % 根据信号功率和信噪比计算高斯白噪声功率
noise = sqrt(noise_power/2) * (randn(size(raw_iq_data_reset0)) + 1j * randn(size(raw_iq_data_reset0))); % 根据噪声功率计算高斯白噪声信号
% raw_iq_data_noise = raw_iq_data_reset0 + noise;
% raw_iq_data_noise = raw_iq_data_reset0;
...
figure;
subplot(2,1,1);
plot(t_tx*1e6,abs(raw_iq_data(2,:,5))); % 画prt 1，通道2的波形图
title('回波信号实部时域波形图 prt2 通道5');
xlabel('时间（us）')
ylabel('信号幅度')

subplot(2,1,2);
plot(t_tx*1e6,abs(raw_iq_data_noise(2,:,5))); % 画prt 1，通道2的波形图
title('回波信号实部时域波形图（带噪声）prt2 通道5');
xlabel('时间（us）')
ylabel('信号幅度')
...
figure; ch=2; m=1;
plot(t_tx*1e6, abs(squeeze(raw_iq_data_noise(m,:,ch)))); hold on; grid on;
...
figure;
plot(abs(pc_out_medium_full(1,:))); % 输出脉冲压缩结果
title('脉压后信号幅度');
xlabel('距离 (m)');
ylabel('幅度');
xline(target_distance, '--r', '目标'); % 在目标距离处画线
