% main_simulate_and_process_v6.m
% 版本: v6.0
...
targets = struct(...
    ...
    'RCS', {50}, ...                % 雷达散射截面 (m^2)
    'ElevationAngle', {30} ...        % 目标俯仰角 (度)
);

SNR_dB = 20; % 信噪比 功率（20dB-100倍，10dB-10倍，3dB-1倍）
...
% %% DBF过程对噪声的影响（噪声在DBF前加入）
% [num_prt, num_points, num_channels] = size(noise);
% % 初始化 DBF 后的噪声矩阵
% dbf_noise = complex(zeros(num_prt, num_points, config.Sig_Config.beam_num));
% 
% % 对每个 PRS 进行处理
% for i = 1:num_prt
%     % 获取每个 PRS 的16个通道噪声
%     noise_16ch = squeeze(noise(i, :, :));  % 这是一个 (point_PRT, 16) 的矩阵，代表16个通道的噪声
% 
%     % 通过DBF系数对每个通道进行加权
%     dbf_noise_single_prt = noise_16ch * DBF_coeffs_data_C.';  % 注意系数是复数矩阵的乘法
% 
%     % 将处理后的结果存入 dbf_noise
%     dbf_noise(i, :, :) = dbf_noise_single_prt;
% end
% 
% % 计算噪声通过DBF后的功率
% dbf_noise_power = mean(abs(dbf_noise).^2, 'all');  % 对每个时间点（快时间维度）求平均功率
% 
% % 可视化噪声通过DBF后的功率分布（以一个波束为例）
% beam_idx = 1;  % 选择第一个波束
% figure;
% plot(squeeze(dbf_noise_power(1, :, beam_idx)));
% title('噪声通过DBF处理后的功率分布');
% xlabel('距离单元');
% ylabel('功率');
...
% --- 6.3 逐波束进行处理 ---
pc_results_16channel = complex(zeros(config.Sig_Config.prtNum, N_total_gate, config.Sig_Config.channel_num));

for c = 1:config.Sig_Config.channel_num
    % 提取当前通道的所有PRT数据
    channel_data = squeeze(raw_iq_data_noise(:,:,c));

    % --- 1. 提取用于处理的、连续的回波信号段 ---
    % 窄脉冲处理段: 从窄脉冲发射结束开始
    seg_start_narrow = num_samples_1 + 1;
    segment_narrow = channel_data(:, seg_start_narrow:end);

    % 中脉冲处理段: 从中脉冲发射结束开始
    seg_start_medium = offset1 + num_samples_2 + 1;
    segment_medium = channel_data(:, seg_start_medium:end);

    % 长脉冲处理段: 从长脉冲发射结束开始
    seg_start_long = offset2 + num_samples_3 + 1;
    segment_long = channel_data(:, seg_start_long:end);
    ...
    pc_results_16channel(:,:,c) = [piece1, piece2, piece3];
end

fprintf('脉冲压缩完成。拼接后的总距离门数为: %d\n', size(pc_results_16channel, 2));
...
% 对13个波束的脉压结果，逐一进行MTD
rdm_16channel = complex(zeros(size(pc_results_16channel)));

for c = 1:config.Sig_Config.channel_num
    % 提取单个波束的脉压数据立方体
    pc_data_single_beam = squeeze(pc_results_16channel(:,:,c));

    % 沿慢时间维(第1维)做FFT，并进行fftshift将零频移到中心
    %rdm_13beam(:,:,b) = fftshift(fft(pc_data_single_beam, [], 1), 1);
    rdm_16channel(:,:,c) = fft(pc_data_single_beam, [], 1);
end
...
figure;
subplot(3,1,1);
plot(t_tx*1e6,real(raw_iq_data(2,:,5))); % 画prt 2，通道5的波形图
...
subplot(3,1,2);
plot(t_tx*1e6,imag(raw_iq_data(2,:,5))); % 画prt 2，通道5的波形图
subtitle('虚部');
xlabel('时间（us）')
ylabel('信号幅度')

subplot(3,1,3);
plot(t_tx*1e6,abs(raw_iq_data(2,:,5))); % 画prt 2，通道5的波形图
subtitle('幅值');
xlabel('时间（us）')
ylabel('信号幅度')
...
figure;
subplot(3,1,1);
plot(t_tx*1e6,real(raw_iq_data_noise(2,:,5))); % 画prt 2，通道5的波形图
...
subplot(3,1,2);
plot(t_tx*1e6,imag(raw_iq_data_noise(2,:,5))); % 画prt 2，通道5的波形图
subtitle('虚部');
xlabel('时间（us）')
ylabel('信号幅度')

subplot(3,1,3);
plot(t_tx*1e6,abs(raw_iq_data_noise(2,:,5))); % 画prt 2，通道5的波形图
subtitle('幅值');
xlabel('时间（us）')
ylabel('信号幅度')
...
% figure;
% subplot(2,1,1);
% plot(t_tx*1e6,abs(pc_results_16channel(2,:,5))); % 画prt 1，通道2的波形图
...
% 
% subplot(2,1,2);
% plot(t_tx*1e6,20*log10(abs(pc_results_16channel(2,:,5))+1e-2)); % 画prt 1，通道2的波形图
% title('回波信号实部时域波形图 prt2 通道5');
% subtitle('无噪声');
% xlabel('时间（us）')
% ylabel('信号幅度')
...
% 画脉冲压缩后距离效果图
figure;
plot(R_rx -24, abs(pc_results_16channel(1,:,5))); 
title('脉压后信号赋值图')
xlabel('距离（m）')
ylabel('信号幅度')
...
figure;
plot(R_rx-24, 20*log10(abs(pc_results_16channel(1,:,5)))); 
title('脉压后信号赋值图（dB）')
xlabel('距离（m）')
ylabel('信号幅度（dB）')
xline(targets.Range,'--r','目标','LabelOrientation','horizontal')
...
subplot(4,1,3)
plot(RX1, abs(pc_out_narrow_full(1,:))); % 输出脉冲压缩结果
subtitle('幅值');
xlabel('距离 (m)');
ylabel('幅度');

subplot(4,1,4)
plot(RX1, 20*log10(abs(pc_out_narrow_full(1,:))+1e-1)); % 输出脉冲压缩结果
subtitle('幅值（dB）');
xlabel('距离 (m)');
ylabel('幅度');
...
%% 全程处理（不截取不拼接距离点）
Channel_data_temp = squeeze(raw_iq_data_noise(:,:,3));

% 中脉冲 (时域卷积)
pc_out_medium_full_temp = complex(zeros(size(Channel_data_temp,1), size(Channel_data_temp,2) + num_samples_2 - 1));
for i = 1:config.Sig_Config.prtNum
    pc_out_medium_full_temp(i,:) = conv(Channel_data_temp(i,:), MF_medium);
end

% 长脉冲 (时域卷积)
pc_out_long_full_temp = complex(zeros(size(Channel_data_temp,1), size(Channel_data_temp,2) + num_samples_3 - 1));
for i = 1:config.Sig_Config.prtNum
    pc_out_long_full_temp(i,:) = conv(Channel_data_temp(i,:), MF_long);
end
