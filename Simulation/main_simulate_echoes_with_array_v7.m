% main_simulate_echoes_with_array_v7.m
% 版本: v7.0
...
targets = struct(...
    'Range', {8000}, ...        % 目标距离 (m)
    'Velocity', {15}, ...       % 目标速度 (m/s, 正为靠近雷达)
    'RCS', {1}, ...             % 雷达散射截面 (m^2)
    'ElevationAngle', {44} ...  % 目标俯仰角 (度)
);
...
% --- 8.1 形成用于检测的和波束 ---
% 为了避免目标恰好落在两个波束之间导致能量分散、不易检测，将相邻两个波束的功率进行非相干累加(直接相加)，
% 形成一个覆盖范围更广、能量更稳定的"和波束"用于后续检测。
beam_idx_A = 9;
beam_idx_B = 10;
rdm_beamA = abs(squeeze(rdm_13beam(:,:,beam_idx_A))); % 左波束
rdm_beamB = abs(squeeze(rdm_13beam(:,:,beam_idx_B))); % 右波束
rdm_for_cfar = rdm_beamA + rdm_beamB;                 % 和波束

fprintf('已将波束 #%d 和 #%d 合成为用于CFAR检测的和波束。\n', beam_idx_A, beam_idx_B);

% --- 8.2 实现 2D-GOCA-CFAR 检测器 ---
% 获取CFAR参数
P_fa = 1e-6;                           % 虚警率，本程序中 P_fa 仅为参考，T_CFAR直接指定
...
for r = (ref_R + guard_R + 1) : (num_R - ref_R - guard_R)
    for v = (ref_V + guard_V + 1) : (num_V - ref_V - guard_V)
        ...
        threshold_map(v, r) = threshold;      % 保存当前单元的门限值，方便后续分析
...
% =========================================================================
fprintf('\n--- 9. 执行目标聚类与最终参数估计 ---\n');
final_targets = []; % 首先，初始化最终目标列表为空

if num_detections > 0
    % --- 9.1 聚类/波束归并 ---
    detections = [detected_v_idx, detected_r_idx];
    all_detections_with_beam = []; % 用于存储带波束号的检测
    % (由于CFAR只在一个和波束上进行，我们需要找到原始波束信息)
    % (为简化，我们先假设CFAR检测点在所有波束中都存在，后续再精细化)
    
    % 注意：下面的聚类代码是基于上一个版本的多波束CFAR输出的
    % 在当前的单波束CFAR代码中，所有点都在一个"波束"上，聚类只会合并距离和速度
    
    cluster_IDs = zeros(num_detections, 1);
    current_cluster_ID = 0;
    max_range_sep = 5; max_vel_sep = 3;
        ...
        if cluster_IDs(i) == 0
            ...
            while ~isempty(points_to_visit)
                ...
                if cluster_IDs(current_point_idx) == 0
                    cluster_IDs(current_point_idx) = current_cluster_ID;
...
if num_clusters > 0
    ...
    for i = 1:num_clusters
        ...
        [~, main_beam_idx] = max(all_beams_power_at_target);

        if main_beam_idx == 1, beam_A_idx = 1; beam_B_idx = 2;
        elseif main_beam_idx == config.Sig_Config.beam_num, beam_A_idx = config.Sig_Config.beam_num - 1; beam_B_idx = config.Sig_Config.beam_num;
        else
            power_left = abs(rdm_13beam(v_idx, r_idx, main_beam_idx - 1));
            power_right = abs(rdm_13beam(v_idx, r_idx, main_beam_idx + 1));
            if power_left > power_right, beam_A_idx = main_beam_idx - 1; beam_B_idx = main_beam_idx;
            else, beam_A_idx = main_beam_idx; beam_B_idx = main_beam_idx + 1;
            end
        end
...
figure;
plot(threshold_map(detected_v_idx(1), :), 'r');
legend('检测门限')
hold on;
plot(rdm_for_cfar(detected_v_idx(1), :),'b');
legend('和波束幅度');
hold off;
title('和波束幅度与检测门限情况');
