% main_simulate_echoes_with_array_v7_3.m
% 版本: v7.3
...
%% 9. 逐点参数估计 (含单脉冲测角) 
% =========================================================================
fprintf('--- 9. 对每个原始点迹进行参数估计 (含峰值样条插值) ---\n');
...
if num_total_raw_detections > 0
    ...
    for i = 1:num_total_raw_detections
        ...
        if length(rCellsFix) < 3 % 样条插值至少需要3个点
            ...
            dataUsed_r = rdm_interp(v_idx, rCellsFix); % 提取窗口内的幅度数据
            % 创建精细网格 (例如，在每个原始单元内插入 interpTimes-1 个点)
            rCellsFixQ = rCellsFix(1) : (1/rInterpTimes) : rCellsFix(end); 
            % 执行三次样条插值
            interpData_r = interp1(rCellsFix, dataUsed_r, rCellsFixQ, 'spline');
            % 在插值后的精细曲线上找到最大值及其对应的精细索引
            [~, I1] = max(interpData_r);
            rCellMax = rCellsFixQ(I1(1)); % 精确的峰值位置索引 (带小数)
        end
        ...
        if length(vCellsFix) < 3
            ...
            dataUsed_v = rdm_interp(vCellsFix, r_idx); % 提取窗口内的幅度数据 (列向量)
            vCellsFixQ = vCellsFix(1) : (1/vInterpTimes) : vCellsFix(end);
            % 执行三次样条插值 (interp1需要行向量作为输入y)
            interpData_v = interp1(vCellsFix, dataUsed_v', vCellsFixQ, 'spline');
            [~, I2] = max(interpData_v);
            vCellMax = vCellsFixQ(I2(1)); % 精确的峰值位置索引 (带小数)
        end
...
fprintf('已完成对 %d 个原始点迹的参数估计 (含峰值样条插值)。\n', num_total_raw_detections);
...
% fprintf('--- 9. 对每个原始点迹进行参数估计 ---\n');
% % 初始化一个结构体数组，用于存储每个原始点迹的完整物理参数
% parameterized_detections = []; 
% 
% if num_total_raw_detections > 0
%     % --- 定义坐标轴和标定数据 ---
%     N_total_gate = size(rdm_13beam, 2);
%     range_axis = (0:N_total_gate-1) * (config.Sig_Config.c / (2 * config.Sig_Config.fs));       % 距离轴
%     v_max = config.Sig_Config.wavelength / (2 * config.Sig_Config.prt);
%     velocity_axis = linspace(-v_max/2, v_max/2, config.Sig_Config.prtNum);                      % 速度轴
%     [num_V, num_R] = size(squeeze(rdm_13beam(:,:,1)));
%     beam_angles_deg = [-16, -9.6, -3.2, 3.2, 9.6, 16, 22.6, 29.2, 36.1, 43.3, 51, 59.6, 70.3];  % 13波束波峰对应的角度
%     k_slopes_LUT = [-4.6391,-4.6888,-4.7578,-4.7891,-4.7214,-4.7513,-5.2343,-5.4529,-5.7323,-6.1685,-7.0256,-8.7612]; % 测角k值
% 
%     % --- 遍历每一个原始点迹 ---
%     for i = 1:num_total_raw_detections
%         v_idx = all_raw_detections(i, 1);   % 速度索引 (整数)
%         r_idx = all_raw_detections(i, 2);   % 距离索引 (整数)
%         pair_idx = all_raw_detections(i, 3);% 波束对索引
%         power = all_raw_detections(i, 4);   % 峰值幅值 (线性)
% 
%         % % --- 距离和速度估计 (直接索引，未插值精测) ---
%         % est_range = range_axis(r_idx);
%         % est_velocity = velocity_axis(v_idx);
% 
%         % --- 内插精测物理量 ---
%         % 注意: 插值需要在功率域进行，并且使用检测该点的那个和波束数据
%         rdm_interp = rdm_for_cfar_all(:,:,pair_idx); % 提取对应的和波束幅值 RDM
% 
%         % --- 距离维抛物线插值 ---
%         if r_idx > 1 && r_idx < num_R % 检查边界
%             y_left_r = rdm_interp(v_idx, r_idx - 1);
%             y_peak_r = rdm_interp(v_idx, r_idx); % = power
%             y_right_r = rdm_interp(v_idx, r_idx + 1);
%             % 计算偏移量 (避免分母为0)
%             denominator_r = y_left_r - 2*y_peak_r + y_right_r;
%             if abs(denominator_r) > 1e-6 % 只有在分母足够大时才计算偏移
%                  delta_r = 0.5 * (y_left_r - y_right_r) / denominator_r;
%             else
%                  delta_r = 0; % 如果分母接近0（峰值太平坦或噪声），则不偏移
%             end
%             % 限制偏移量在合理范围内 (-0.5 到 +0.5)
%             delta_r = max(min(delta_r, 0.5), -0.5);
%             r_idx_interp = r_idx + delta_r; % 得到插值后的距离索引
%         else
%             r_idx_interp = r_idx; % 边界点不进行插值
%         end
% 
%         % --- 速度维抛物线插值 ---
%         if v_idx > 1 && v_idx < num_V % 检查边界
%             y_lower_v = rdm_interp(v_idx - 1, r_idx);
%             y_peak_v = rdm_interp(v_idx, r_idx); % = power
%             y_upper_v = rdm_interp(v_idx + 1, r_idx);
%             denominator_v = y_lower_v - 2*y_peak_v + y_upper_v;
%             if abs(denominator_v) > 1e-6
%                 delta_v = 0.5 * (y_lower_v - y_upper_v) / denominator_v;
%             else
%                 delta_v = 0;
%             end
%             delta_v = max(min(delta_v, 0.5), -0.5);
%             v_idx_interp = v_idx + delta_v; % 得到插值后的速度索引
%         else
%             v_idx_interp = v_idx; % 边界点不进行插值
%         end
% 
%         % --- 使用插值后的索引计算精确物理量 ---
%         est_range = interp1(1:num_R, range_axis, r_idx_interp);
%         est_velocity = interp1(1:num_V, velocity_axis, v_idx_interp);
% 
%         % --- 角度估计 (单脉冲测角) ---
%         beam_A_idx = pair_idx;
%         beam_B_idx = pair_idx + 1;
...
%         parameterized_detections(i).PairIndex = pair_idx;  % 保存波束对所属信息
%     end
% end
% fprintf('已完成对 %d 个原始点迹的参数估计。\n', num_total_raw_detections);

%% 10. 最终聚类 (基于物理参数)
% =========================================================================
fprintf('--- 10. 执行最终聚类 (基于物理参数) ---\n');
final_targets = [];
