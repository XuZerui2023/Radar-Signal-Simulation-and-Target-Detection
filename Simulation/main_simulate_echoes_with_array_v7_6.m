% main_simulate_echoes_with_array_v7_6.m
% 版本: v7.6
...
% --- 定义目标 1 ---
targets(1).Range = 8000;         % 目标1 距离 (m)
targets(1).Velocity = 15;         % 目标1 速度 (m/s)
targets(1).RCS = 1;               % 目标1 雷达散射截面 (m^2)
targets(1).ElevationAngle = 10;   % 目标1 俯仰角 (度)
targets(1).SNR_dB = 15;           % 目标1 信噪比 (dB)

% --- 定义目标 2 ---
targets(2).Range = 10000;         % 目标2 距离 (m)
targets(2).Velocity = 15.4;       % 目标2 速度 (m/s)
targets(2).RCS = 1;               % 目标2 雷达散射截面 (m^2)
targets(2).ElevationAngle = 10;   % 目标2 俯仰角 (度)
targets(2).SNR_dB = 15;           % 目标2 信噪比 (dB)
...
cluster_params.max_vel_sep = 0.2;   % 速度维上，点被视为同一簇的最大间隔 (米/秒)
